		...
		this.each(function () {
			...
			function initAudioEvents() {
				...
				audioEle.addEventListener('timeupdate', function () {
					...
					// console.log('time', audioEle.currentTime);
					// console.log('width', $played.width());
					// console.log('offset', $played.offset().left);

					// $seek.css({
					// 	'left': ($played.offset().left + $played.width() - $seek.width() / 2) + 'px'
					// });
					// $seek.css({
					// 	'left': (($played.offset().left)/2 + $played.width() + $seek.width()/2) + 'px'
					// });
					if(window.matchMedia('(max-width:767px)').matches){
						$seek.css({
							'left': ($played.width() + 20) + 'px'
						});

					} else {
						$seek.css({
						'left': ($played.width() + 136) + 'px'
						});
					}
					// $seek.css({
					// 	'left': ($played.width() + 136) + 'px'
